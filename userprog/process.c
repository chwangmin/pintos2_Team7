...
process_exit (void) {
	...
	/* project 3 */
	// supplemental_page_table_init(&thread_current()->spt); // 초기화해주지 않으면 exec 실패
...
lazy_load_segment (struct page *page, void *aux) {
	...
	memset(page->frame->kva + lazy_load_arg->read_bytes, 0, lazy_load_arg->zero_bytes); // 남는 공간 즉 zero_byte 만큼은 0으로 초기화
...
load_segment (struct file *file, off_t ofs, uint8_t *upage,
		uint32_t read_bytes, uint32_t zero_bytes, bool writable) {
	ASSERT ((read_bytes + zero_bytes) % PGSIZE == 0);			// 4KB로 안나뉘면 read_byte + zero_byte은 잘못 되었다.
	ASSERT (pg_ofs (upage) == 0);								// 4KB 초과면 이것은 잘못 되었다.
	ASSERT (ofs % PGSIZE == 0);									// 4KB로 안나뉘면 ofs은 잘못 되었다.

	while (read_bytes > 0 || zero_bytes > 0) {					// 읽기바이트의 크기가 0보다 크거나 나머지 바이트가 0보다 크면 반복
		/* Do calculate how to fill this page.
		 * We will read PAGE_READ_BYTES bytes from FILE
		 * and zero the final PAGE_ZERO_BYTES bytes. */
		size_t page_read_bytes = read_bytes < PGSIZE ? read_bytes : PGSIZE;		// read_byte가 PGSIZE보다 작으면 read_byte 크면 4KB
		size_t page_zero_bytes = PGSIZE - page_read_bytes;						// 남는 zero_byte는 남는 부분을 할당.

		/* TODO: Set up aux to pass information to the lazy_load_segment. */
		struct lazy_load_arg *lazy_load_arg = (struct lazy_load_arg *)malloc(sizeof(struct lazy_load_arg));		// lazy_load_arg의 크기를 지정해서 프로세스메모리 즉 가상메모리(커널 풀)에 할당한다.
		lazy_load_arg->file = file;																				// lazy_load_arg에 파일 객체 저장.
		lazy_load_arg->ofs = ofs;																				// 읽기 시작할 위치
		lazy_load_arg->read_bytes = page_read_bytes;															// 읽을 byte와
		lazy_load_arg->zero_bytes = page_zero_bytes;															// 읽고 나머지 byte를 zero로 만든다.
		if (!vm_alloc_page_with_initializer(VM_ANON, upage,
											writable, lazy_load_segment, lazy_load_arg))						// 1,upage, writable, lazy_load_segment, lazy_load_arg를 넣어
																												// 페이지를 하나 생성해서 
			return false;

		/* Advance. */
		read_bytes -= page_read_bytes;			// 읽기 바이트를 빼줌 만약 읽기 바이트가 4KB보다 크면 read_bytes - 4KB일 거임.
		zero_bytes -= page_zero_bytes;			// 남는 바이트를 빼줌 만약 읽는 바이트가 4KB보다 크면 값이 그대로일 거임.
		upage += PGSIZE;						// upage를 4KB만큼 추가해줌
		ofs += page_read_bytes;					// ofs이란 시작 지점에서 추가를 한 바이트 즉, page_read_bytes 만큼 추가시켜줌.
	}
	return true;
}
